...
if (!callAutomationConfig) {
  console.warn('Call Automation configuration is not set');
}
...
export async function SummarizeConversation(
  ...
  const requestResponse = await requestConversationSummary(
    (input as unknown) as ConversationSummaryInput,
    language ?? 'en'
  );
